...
cameraManager::cameraManager()
{
	...
	_worldImage = IMAGEMANAGER->addImage("worldMap", 5000, 5000);
	...
	_cameraWorldSizeX = 2160;
	_cameraWorldSizeY = 1800;
...
void cameraManager::Camera_WorldDC_Shake()
{
	...
	set_CameraXY( _cameraX + WINSIZEX / 2 + RND->getFromIntTo(-3, 3), _cameraY + WINSIZEY / 2 + RND->getFromIntTo(-3, 3));
	set_CameraXY( _cameraX + WINSIZEX / 2 + RND->getFromIntTo(-3, 3), _cameraY + WINSIZEY / 2 + RND->getFromIntTo(-3, 3));
	set_CameraXY( _cameraX + WINSIZEX / 2 + RND->getFromIntTo(-3, 3), _cameraY + WINSIZEY / 2 + RND->getFromIntTo(-3, 3));
	
}

void cameraManager::Strong_Camera_WorldDC_Shake()
{
	set_CameraXY(_cameraX + WINSIZEX / 2 + RND->getFromIntTo(-10, 10), _cameraY + WINSIZEY / 2 + RND->getFromIntTo(-10, 10));
	set_CameraXY(_cameraX + WINSIZEX / 2 + RND->getFromIntTo(-10, 10), _cameraY + WINSIZEY / 2 + RND->getFromIntTo(-10, 10));
	set_CameraXY(_cameraX + WINSIZEX / 2 + RND->getFromIntTo(-10, 10), _cameraY + WINSIZEY / 2 + RND->getFromIntTo(-10, 10));
